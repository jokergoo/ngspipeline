...
# fastqc
###################################################################
$pipeline->set_job_name("$sample_id"."_star_fastqc_r1_$i");
$qid->{fastqc_r1} = $pipeline->star->fastqc(
		fastq      => $r1_fastq,
		output_dir => "$pipeline->{dir}/fastqc_r1_$i"
);

$pipeline->set_job_name("$sample_id"."_star_fastqc_r2_$i");
...
# trim
####################################################################
$pipeline->set_job_name("$sample_id"."_star_trimmed_$i");
...
###################################################################
$pipeline->set_job_name("$sample_id"."_star_fastqc_r1_trimmed_$i");
...
$pipeline->set_job_name("$sample_id"."_star_fastqc_r2_trimmed_$i");
$pipeline->set_job_dependency($qid->{trim});
...
# alignment
##################################################################
$pipeline->set_job_name("$sample_id"."_star_align");
$pipeline->set_job_dependency($qid->{trim});
